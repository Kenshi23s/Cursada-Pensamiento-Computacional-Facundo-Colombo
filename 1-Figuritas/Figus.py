...
def chance_llenar_album(resultados_album_lleno, cantidad_maxima_figus):
    ...
    return (resultadosFavorables / cantidad_maxima_figus) * 100.0 #float
...
def llenar_y_devolver_chance(figus_total_album, cantidad_maxima_figus, n_albumes):
    ...
    album = [0] * figus_total_album

    repeticionActual =n_albumes
    while(repeticionActual > 0):
         

    chance_llenar_album(album,cantidad_maxima_figus)
    return 0